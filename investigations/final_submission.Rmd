...
Apply to dataset
```{r}
pca_result <- pca(as.matrix(test_data), 2)

ggplot(data=data.frame(pca_result$reduced_data), aes(x=X1, y=X2)) + geom_point(aes(colour=test_classes)) + stat_function(fun=sp)
```
