...
```{r}
pca_reduced_test_data <- t(apply(as.matrix(test_data),1, function(x) x %*% pca_result$reduction_matrix))
```
...
```{r}
lr_embedded_preds <- round(1.0 / (1.0 + exp(-cbind(1, as.matrix(embedded_test_data[, c(1, 2)])) %*% start)))
