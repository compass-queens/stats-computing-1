...
```{r}
# MLE, BFGS
lr_mle_bfgs <- logistic_regression(Xtrain, training_classes, cost = "MLE", 
                                   method = "BFGS")
yhat_mle_bfgs <- predict(lr_mle_bfgs, Xtest)
acc_mle_bfgs <- calc_accuracy(test_classes, yhat_mle_bfgs)
# MLE, NEWTON
lr_mle_nm <- logistic_regression(Xtrain, training_classes, cost = "MLE", 
                                 method = "NEWTON", niter = 250)
yhat_mle_nm <- predict(lr_mle_nm, Xtest)
acc_mle_nm <- calc_accuracy(test_classes, yhat_mle_nm)
# MLE, GRADIENT ASCENT
lr_mle_ga <- logistic_regression(Xtrain, training_classes, cost = "MLE", 
                                 method = "GA", niter = 1000)
...
```{r}
samples <- rwmh_multivariate_log(start,
  niter = 50000, log_posterior_unnormalized,
...
```{r}
lr_embedded_preds <- round(1 / (1 + exp(-cbind(1,as.matrix(embedded_test_data[, c(1, 2)]))
%*% start)))
