...
pairwise_dist_median <- function(x){
  ...
  for(i in 1:(n-1)){
    for(j in (i+1):n){
      ...
      dist[total_iterations] <- norm(x[i,]-x[j,], "2")^2
    }
  }
  return(median(dist))
}
...
# input: centroid and sigma
# output: function phi_i(x)
rbf <- function(centroid,sigma){
  rbf_in <- function(x) {
    phi <- exp(-norm(x-centroid, "2")^2/(sigma*sigma))
    return(phi)
  }
  return(rbf_in)
}

# rbf2 <- function(x, centroid, sigma){
#   phi <- exp(-norm(x-centroid, "2")^2/(sigma*sigma))
#   return(phi)
# }
```
...
create_data_matrix <- function(X, n) {
  num_cols <- dim(X)[2]
  #todo fix n = 1
  centroid_indices <- sample(1:num_cols, n)
  centroids <- X[,centroid_indices]
  sigma <- pairwise_dist_median(X)
   
  big_phi_x <- matrix(, nrow=n, ncol=num_cols)
  i <- 0
  for(i in 1:n) {
    print(centroid)
    phi_i <- rbf(centroid, sigma)
    phi_i_x <- apply(X, 2, phi_i)
    big_phi_x[i,] <-  phi_i_x
    i <- i + 1
  }
...
phi <- function(X,n){
  ...
  for(i in centroid_indices){
    centroid <- X[,i]
    norm_squared <- apply(X, 2, function(x) norm(x-centroid,"2")^2)
    phi_i <- exp(-norm_squared/(sigma*sigma))
    phi <- cbind(phi, phi_i)
  }
    return(phi[,-1])
}
